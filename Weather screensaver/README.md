...
4. Нужна прошивка ZiFi `s3-native-0.6.93` или новее (обновляется плагином
   ...
   В 0.6.92 место искалось только по почтовому индексу.
...
Wild Commander, часов Mr.Gluk и ESP на другом конце UART. Кадр из видеостраниц сравнивается
...
[`tests/test_weather_parse.py`](../tests/test_weather_parse.py) в корне
...
| `tools/prepare_unreal.py` | стенд в эмуляторе Unreal с моделью ESP |
| [`index.html`](index.html) | исходный макет экрана |

Общее с заставкой для VDAC2 ([`WEATHER2.WMF`](../Weather%20screensaver%20VDAC2))
...
| `harness.py` | машинный стенд: Z80, модель WC, часов и ESP |
| `unreal_bench.py`, `esp_model.py` | стенд в эмуляторе Unreal и модель ESP для него |
