...
; FTP-сервер и дальше только отвечает на файловые запросы. Ни одной текстовой
; AT-команды и никакого разбора асинхронных строк: сеть, TCP и сам протокол FTP
; целиком на стороне нативной прошивки ESP32-S3. Разбор текста на Z80 был главным источником
; зависаний, поэтому от него отказались полностью.
;
...
.network_error:
        ; LinkError: 1 — нет ZiFi, 2 — ESP молчит, 3 — Wi-Fi/IP, 4 — FTP не поднялся.
        ld a,(LinkError)
        cp 1
        ld hl,UiErrorNoZifi
        jr z,.fatal
        ...
        jr .fatal
.err_server:
        ; Причину отказа FTP присылает ESP пакетом #EE — показываем её.
        ld hl,UiErrorServer
        ld a,(ProtoErr)
        or a
        jr z,.fatal
...
.info_done:
        ...
        ; IP выводим СРАЗУ: если дальше сорвётся FTP, пользователь всё равно
        ...
        ; Просим ESP поднять FTP-сервер. Вместе с портом отдаём логин и пароль:
        ; в прошивке ESP они не зашиты, поэтому менять их можно правкой плагина,
        ; без перепрошивки модуля.
        ld a,CMD_FTP_START
        ld hl,LinkFtpStart
        ld bc,LinkFtpStartLen
        call Proto_Send
        ld a,RESP_ACK
        ld de,LINK_TRY
        call Proto_WaitCmd
...
; Состояние управляющего FTP-клиента: 0=нет, 1=подключён, 2=авторизован.
...
; Последняя реальная FTP-команда. ESP уже удалил аргументы USER/PASS, поэтому
; пароль никогда не появляется на экране. Имена файлов приходят в UTF-8;
; перед выводом переводим их в внутреннюю кодировку Wild Commander.
FtpEvent_Command:
        ld hl,(ProtoRxLen)
        ld a,h
        or l
...
; Тело команды FTP_START: порт (младший байт первым), затем логин и пароль,
; каждый с завершающим нулём. Менять учётные данные здесь.
LinkFtpStart:
LinkFtpPort:    dw 21
                db "zx",0                ; логин
                db "zx",0                ; пароль
LinkFtpStartLen equ $-LinkFtpStart
LinkIniMsg:     db "ini=",0
