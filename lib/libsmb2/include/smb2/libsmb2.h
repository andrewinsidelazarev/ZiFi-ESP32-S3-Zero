...
/* TreeId запроса, который сервер обрабатывает в данный момент. */
uint32_t smb2_get_current_tree_id(struct smb2_context *smb2);
...
struct smb2_server_request_handlers {
        ...
        /* Called from the listener task once per service-loop iteration.  A
         * server can use it to complete requests that previously returned an
...
struct smb2_server {
        ...
        char domain[128];
        int fd;
        /* ZiFi останавливает сервер из другой задачи FreeRTOS. Флаг проверяется
         * после каждого короткого select(), поэтому закрывать сокет из чужой
         * задачи и рисковать повторным close() не требуется. */
