...
smb2_bind_and_listen(const uint16_t port, const int max_connections, int *out_fd)
{
        ...
        /* ZiFi: повторный запуск плагина должен сразу снова занять порт 445,
         * даже если прежнее клиентское соединение ещё находится в TIME_WAIT. */
        {
                int reuse_address = 1;
...
int smb2_accept_connection_async(const int fd, const int to_msec, smb2_accepted_cb cb, void *cb_data)
{
        ...
        if (err > 0) {
                ...
                if (clientfd >= 0) {
                        ...
                        /* ZiFi: the listener is non-blocking.  A short-lived
                         * port probe may disappear between poll() and accept();
                         * this is not a fatal listener error. */
#if defined(_WIN32) || defined(_MSC_VER)
                ...
                /* Interrupted poll is equally harmless and must not tear down
                 * port 445. */
#if defined(_WIN32) || defined(_MSC_VER)
                err = WSAGetLastError() == WSAEINTR ? 0 : -EIO;
#else
                err = errno == EINTR ? 0 : -EIO;
#endif
        }
