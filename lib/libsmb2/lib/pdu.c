...
uint32_t
smb2_get_current_tree_id(struct smb2_context *smb2)
{
        if (smb2 == NULL) {
                return 0;
        }
        return smb2_tree_id(smb2);
}
...
smb2_correlate_reply(struct smb2_context *smb2, struct smb2_pdu *pdu)
{
        ...
        if (req_pdu == NULL) {
                 ...
                 * progress until the whole queue completes.  Three initial
                 * credits are still needed for Explorer's compound metadata
                 * requests.  Afterwards grant back exactly what each request
                 * consumed, keeping the connection at that fixed depth. */
                if (smb2_is_server(smb2)) {
                ...
                */
                if ((pdu->header.flags & SMB2_FLAGS_ASYNC_COMMAND) &&
                        ...
                        /* The application owns the lifetime of a server-side
                         * asynchronous operation after STATUS_PENDING.  Keep
                         * the request available for the final response instead
                         * of expiring it at the context's ordinary PDU timeout.
                         * The server must provide its own operation watchdog. */
                        req_pdu->timeout = 0;
               } else {
...
smb2_queue_pdu(struct smb2_context *smb2, struct smb2_pdu *pdu)
{
        ...
        for (p = pdu; p; p = p->next_compound) {
                ...
                /* MS-SMB2 3.3.4.2: the interim asynchronous response SHOULD
