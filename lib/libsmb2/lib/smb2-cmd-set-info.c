                            ...
                            struct smb2_pdu *pdu)
{
        ...
        /* Серверный обработчик ZiFi разбирает несколько нужных Windows классов
         * прямо из wire-буфера. Клиентский режим passthrough для этого включать
         * нельзя: он отключил бы штатное кодирование QUERY_INFO и READDIR. */
