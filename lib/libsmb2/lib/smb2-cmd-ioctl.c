                          ...
                          struct smb2_ioctl_reply *rep)
{
        ...
        header_len = SMB2_IOCTL_REPLY_SIZE & 0xfffffffe;
        buf = calloc(header_len, sizeof(uint8_t));
        if (buf == NULL) {
                smb2_set_error(smb2, "Failed to allocate ioctl reply buffer");
                return -1;
        }

        iov = smb2_add_iovector(smb2, &pdu->out, buf, header_len, free);
        if (iov == NULL) {
                return -1;
        }
        ...
        if (rep->output_count) {
                ...
                case SMB2_FSCTL_VALIDATE_NEGOTIATE_INFO:
                        ...
                        */
                        output_len = SMB2_IOCTL_VALIDIATE_NEGOTIATE_INFO_SIZE;
                        break;
                case SMB2_FSCTL_PIPE_TRANSCEIVE:
                case SMB2_FSCTL_PIPE_PEEK:
                        /* ZiFi srvsvc возвращает уже закодированный
                         * DCE/RPC буфер. Его нельзя пытаться транскодировать. */
                        output_len = rep->output_count;
                        break;
                default:
                        if (smb2->passthrough) {
                ...
                buf = malloc(PAD_TO_64BIT(output_len));
                if (buf == NULL) {
                        smb2_set_error(smb2, "Failed to allocate ioctl output");
                        return -1;
                }
                memset(buf, 0, PAD_TO_64BIT(output_len));
                ioctlv = smb2_add_iovector(smb2, &pdu->out, buf,
                                            output_len, free);
                if (ioctlv == NULL) {
                        return -1;
                }
        ...
        /* У reply-структуры нет указателя на input, поэтому на проводе эта
         * область всегда отсутствует. Нулевые области обязаны иметь offset 0.
         * Раньше переменная len всё ещё содержала 48-байтный размер заголовка,
         * и пустой PIPE_WAIT ошибочно объявлял OutputCount=48 без тела. */
        smb2_set_uint32(iov, 24, 0);
        smb2_set_uint32(iov, 28, 0);
        smb2_set_uint32(iov, 32, output_len == 0 ? 0 :
                        SMB2_HEADER_SIZE + header_len);
                         ...
                         struct smb2_pdu *pdu)
{
        ...
        /* A zero-length IOCTL has no variable decode pass, so input must
         * already be NULL when the server callback receives the request. */
                            ...
                            struct smb2_pdu *pdu)
{
        ...
        case SMB2_FSCTL_VALIDATE_NEGOTIATE_INFO:
                ...
                break;
        case SMB2_FSCTL_PIPE_TRANSCEIVE:
        case SMB2_FSCTL_PIPE_WAIT:
        case SMB2_FSCTL_PIPE_PEEK:
                /* Серверному обработчику нужны точные wire-байты.
                 * Копия живёт до окончания callback и освобождается там. */
                ptr = smb2_alloc_init(smb2, req->input_count);
                if (ptr == NULL) {
                        return -ENOMEM;
                }
