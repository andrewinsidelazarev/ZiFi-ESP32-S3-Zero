...
int
smb2_decode_file_ea_info(struct smb2_context *smb2,
                         void *memctx,
                         struct smb2_file_ea_info *fs,
                         struct smb2_iovec *vec)
{
        return smb2_get_uint32(vec, 0, &fs->ea_size);
}

int
smb2_encode_file_ea_info(struct smb2_context *smb2,
                         struct smb2_file_ea_info *fs,
                         struct smb2_iovec *vec)
{
        if (smb2_set_uint32(vec, 0, fs->ea_size)) {
                return -1;
        }
...
int
smb2_encode_file_internal_info(struct smb2_context *smb2,
                               struct smb2_file_internal_info *fs,
                               struct smb2_iovec *vec)
{
        smb2_set_uint64(vec, 0, fs->index_number);
        return 8;
}
