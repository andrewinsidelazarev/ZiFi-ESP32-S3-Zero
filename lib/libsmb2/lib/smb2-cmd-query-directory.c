                ...
                if (!smb2->passthrough) {
                        ...
                        do {
                                ...
                                case SMB2_FILE_FULL_DIRECTORY_INFORMATION:
                                        fs_size = PAD_TO_64BIT(SMB2_FILE_FULL_DIRECTORY_INFORMATION_SIZE + fname_len);
                                        break;
                                case SMB2_FILE_BOTH_DIRECTORY_INFORMATION:
                                        fs_size = PAD_TO_64BIT(SMB2_FILE_BOTH_DIRECTORY_INFORMATION_SIZE + fname_len);
                                        break;
        ...
        if (smb2->passthrough) {
                ...
                do {
                        ...
                        if (in_remain >= SMB2_FILEID_BOTH_DIRECTORY_INFORMATION_SIZE) {
                                /* NextEntryOffset is relative to the start of
                                 * this entry, not to the output buffer.  The
                                 * old absolute value happened to work only
                                 * while ZiFi returned one entry per reply. */
                        ...
                        case SMB2_FILE_FULL_DIRECTORY_INFORMATION:
                                /*
                                 * Проводник Windows запрашивает обычный класс 0x02.
                                 * В нём после EaSize сразу начинается имя: полей
                                 * Reserved и FileId из класса 0x26 здесь нет.
                                 */
                                fs_size = PAD_TO_64BIT(SMB2_FILE_FULL_DIRECTORY_INFORMATION_SIZE + fname_len);
                                ...
                                if (name && fname_len > 0) {
                                        memcpy(buf + offset + SMB2_FILE_FULL_DIRECTORY_INFORMATION_SIZE, &name->val[0], fname_len);
                                }
                                break;
                        case SMB2_FILE_BOTH_DIRECTORY_INFORMATION:
                                /*
                                 * Современный Windows-клиент использует класс 0x03
                                 * для обычного перечисления каталога. Он похож на
                                 * класс 0x25, но после ShortName сразу, с байта 94,
                                 * начинается имя: Reserved2 и FileId здесь нет.
                                 */
                                fs_size = PAD_TO_64BIT(SMB2_FILE_BOTH_DIRECTORY_INFORMATION_SIZE + fname_len);
                                smb2_set_uint32(iov, offset + 4, fs->file_index);
                                smb2_set_uint64(iov, offset + 8, smb2_timeval_to_win(&fs->creation_time));
                                ...
                                if (name && fname_len > 0) {
                                        memcpy(buf + offset + SMB2_FILE_BOTH_DIRECTORY_INFORMATION_SIZE, &name->val[0], fname_len);
                                }
                                break;
                        case SMB2_FILE_ID_FULL_DIRECTORY_INFORMATION:
                                fs_size = PAD_TO_64BIT(SMB2_FILEID_FULL_DIRECTORY_INFORMATION_SIZE + fname_len);
                                   ...
                                   struct smb2_pdu *pdu)
{
        ...
        /* ZiFi: структуру обязательно обнуляем. При запросе без маски имени
         * функция возвращает управление, не тронув req->name, и обработчик
         * запроса затем видит в нём мусор из кучи, проходит проверку
         * if (req->name) и освобождает произвольный адрес. На ПК это access
         * violation, на ESP32 — паника с reset_reason=4 без всяких следов. */
