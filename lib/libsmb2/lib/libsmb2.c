...
smb2_create_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        if (!ret) {
                memcpy(smb2->last_file_id, rep.file_id, SMB2_FD_SIZE);
                smb2->last_file_id_valid = 1;
                pdu = smb2_cmd_create_reply_async(smb2, &rep, NULL, cb_data);
        }
        else if (ret < 0) {
                smb2->last_file_id_valid = 0;
                memset(&err, 0, sizeof(err));
                pdu = smb2_cmd_error_reply_async(smb2,
...
smb2_query_directory_request_cb(struct smb2_server *server, struct smb2_context *smb2, void *command_data, void *cb_data)
{
        ...
        struct smb2_error_reply err;
        struct smb2_pdu *pdu = NULL;
        const char *name_to_free;
        int ret = -1;

        memset(&rep, 0, sizeof(rep));
        memset(&err, 0, sizeof(err));

        /* ZiFi: имя запоминаем ДО вызова обработчика. Обработчик вправе сам
         * ответить клиенту и вернуть 1 — тогда входящий PDU вместе со своим
         * payload уже уничтожен, и прежнее чтение req->name после возврата шло
         * из освобождённой памяти. На ПК это access violation, на ESP32 —
         * паника reset_reason=4 без единого следа в логе. */
        name_to_free = req->name;
        ...
        if (name_to_free) {
                smb2_free_data(smb2, discard_const(name_to_free));
        }
        if (pdu != NULL) {
                smb2_set_pdu_message_id(smb2, pdu, smb2->message_id);
                smb2_queue_pdu(smb2, pdu);
...
static uint8_t *
smb2_request_file_id(enum smb2_command command, void *command_data)
{
        if (!command_data) {
                return NULL;
        }
        switch (command) {
        case SMB2_CLOSE:
                return ((struct smb2_close_request *)command_data)->file_id;
        case SMB2_FLUSH:
                return ((struct smb2_flush_request *)command_data)->file_id;
        case SMB2_READ:
                return ((struct smb2_read_request *)command_data)->file_id;
        case SMB2_WRITE:
                return ((struct smb2_write_request *)command_data)->file_id;
        case SMB2_LOCK:
                return ((struct smb2_lock_request *)command_data)->file_id;
        case SMB2_IOCTL:
                return ((struct smb2_ioctl_request *)command_data)->file_id;
        case SMB2_QUERY_DIRECTORY:
                return ((struct smb2_query_directory_request *)command_data)->file_id;
        case SMB2_CHANGE_NOTIFY:
                return ((struct smb2_change_notify_request *)command_data)->file_id;
        case SMB2_QUERY_INFO:
                return ((struct smb2_query_info_request *)command_data)->file_id;
        case SMB2_SET_INFO:
                return ((struct smb2_set_info_request *)command_data)->file_id;
        default:
                return NULL;
        }
}
...
static int
smb2_file_id_is_compound_alias(const uint8_t *file_id)
{
        size_t index;

        for (index = 0; index < SMB2_FD_SIZE; ++index) {
                if (file_id[index] != 0xff) {
                        return 0;
                }
        }
        return 1;
...
static void
smb2_prepare_related_file_id(struct smb2_context *smb2,
                             enum smb2_command command,
                             void *command_data)
{
        uint8_t *file_id = smb2_request_file_id(command, command_data);
        const int related =
                (smb2->pdu->header.flags & SMB2_FLAGS_RELATED_OPERATIONS) != 0;
        ...
        if (related && smb2->last_file_id_valid &&
            smb2_file_id_is_compound_alias(file_id)) {
                /* MS-SMB2 3.3.5.2.7.2: a related operation uses the FileId
                 * consumed or generated by the immediately preceding
                 * operation when its on-wire FileId is the all-ones alias.
                 * Resolve it inside this connection instead of leaving that
                 * alias to global application state. */
                memcpy(file_id, smb2->last_file_id, SMB2_FD_SIZE);
        }
        if (!smb2_file_id_is_compound_alias(file_id)) {
                memcpy(smb2->last_file_id, file_id, SMB2_FD_SIZE);
                smb2->last_file_id_valid = 1;
        }
}
...
smb2_general_client_request_cb(struct smb2_context *smb2, int status, void *command_data, void *cb_data)
{
        ...
        smb2_prepare_related_file_id(smb2, smb2->pdu->header.command,
                                     command_data);
...
/* ZiFi: remove every disconnected context that belongs to this listener.
 * smb2_init_context() adds a new context at the head of the global list.  The
 * old code accepted a replacement connection first and then removed only one
 * dead context.  Consequently the just-rejected replacement was removed while
 * the older dead client remained in the application client slot forever. */
static void
smb2_cull_disconnected_server_contexts(struct smb2_server *server)
{
        struct smb2_context *smb2;

        for (;;) {
                struct smb2_context *victim = NULL;

                for (smb2 = smb2_active_contexts(); smb2; smb2 = smb2->next) {
                        if (smb2->owning_server == server &&
                            !SMB2_VALID_SOCKET(smb2_get_fd(smb2))) {
                                victim = smb2;
                                break;
                        }
                }
                ...
                if (server->handlers && server->handlers->destruction_event) {
                        server->handlers->destruction_event(server, victim);
                }
                smb2_destroy_context(victim);
        }
}
...
int smb2_serve_port(struct smb2_server *server, const int max_connections, smb2_client_connection cb, void *cb_data)
{
        ...
        do {
                 ...
                 * when select() timed out and no new socket is waiting. */
                smb2_cull_disconnected_server_contexts(server);

                /* ZiFi: finish asynchronous VFS requests on the same task
                 * that owns every smb2_context.  This keeps the library
                 * single-threaded while core 1 performs slow UART/SD work. */
                if (server->handlers && server->handlers->service_event &&
                    server->handlers->service_event(server) < 0) {
                        err = -1;
                        break;
                }
                ...
                */
                ...
                if (ready > 0) {
                        ...
                        /* A ready client may have closed itself in smb2_service().
                         * Notify the application before accepting its replacement. */
                        smb2_cull_disconnected_server_contexts(server);
                        ...
                        if (FD_ISSET(server->fd, &rfds)) {
                                ...
                                if (!err && smb2) {
                                        /* Mark ownership immediately so every allocation-failure
                                         * path can be culled on the next loop iteration. */
                                        ...
                                        if (c_data == NULL) {
                                                smb2_set_error(smb2, "Failed to allocate connect_data");
                                                smb2_close_context(smb2);
                                                /* ZiFi: после нехватки памяти нельзя разыменовывать
                                                 * нулевой c_data; listener должен пережить отказ клиента. */
                                                continue;
                                        }
        ...
        while (smb2_active_contexts()) {
                ...
                if (smb2->owning_server == server && server->handlers &&
