                              ...
                              struct smb2_negotiate_reply *rep)
{
        ...
        /* Для диалектов до SMB 3.1.1 эти четыре байта зарезервированы и
         * обязаны быть нулевыми. Ненулевое смещение при count==0 терпят
         * некоторые библиотеки, но системные клиенты его отвергают. */
        rep->negotiate_context_offset = rep->negotiate_context_count
                ? len + seclen + SMB2_HEADER_SIZE : 0;
