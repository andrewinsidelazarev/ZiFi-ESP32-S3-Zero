...
SMB объявляет Windows READ и WRITE по 64 КиБ. Реальный Windows CopyFile/FAR в
аппаратном тесте передавал WRITE именно такого размера и менял порядок их
смещений. Асинхронные READ/WRITE получают interim `STATUS_PENDING`, если не
закончены за 30 секунд. `SUCCESS`
...
`s3-native-0.6.89` с выключенным диагностическим кольцевым журналом
...
скопированных байт.
Сервер согласует MaxWrite 64 КиБ: каждый WRITE занимает один credit и один
фиксированный PSRAM-слот. Реальная проверка показала, что Windows передаёт
64-КиБ WRITE и может менять порядок диапазонов. Первый тестовый образ 0.6.86 с
`SecurityMode=0` был отвергнут Windows при NEGOTIATE. В исправленном образе
0.6.86 сервер правильно объявляет signing доступным, но не обязательным (`SecurityMode=1`),
...
выданному `AsyncId`.
В `.89` сервер объявляет file leasing и выдаёт запрошенную `RWH` lease только
единственному эксклюзивному Open нового или перезаписываемого файла назначения.
Пока такой Open жив, второй Open того же пути получает
`STATUS_SHARING_VIOLATION`, поэтому серверу не приходится имитировать
непосланный lease-break. Directory leases и Durable/Persistent Handles не
объявляются. В аппаратном прогоне Windows → ZX Evolution индикатор CopyFile
двигался шагами по 512 КиБ и прошёл прежнюю границу 4 МиБ без зависания.
...
Фиксированное окно из восьми кредитов позволяет принять до восьми 64-КиБ
запросов. Восемь PSRAM-слотов имеют тот же фиксированный размер. Финальный
ответ WRITE приходит после физического завершения,
поэтому в фоне нет подтверждённого write-back хвоста перед `FLUSH`/`CLOSE`, а
незавершённый WRITE остаётся доступен для точного CANCEL. Любое изменение через
`FileEndOfFileInformation`, включая рост, входит в общую физическую FIFO:
сервер посылает interim `STATUS_PENDING`, выполняет `SET_EOF32` и только затем
возвращает финальный `SUCCESS`. Промежуточный `FLUSH` на 4-МиБ границе CopyFile
также становится FIFO-барьером: его финальный ответ приходит после всех ранее
принятых WRITE и применения отложенных метаданных.
Служебный Open, который Windows CopyFile делает ровно после 4 МиБ с доступом
`READ_ATTRIBUTES|SYNCHRONIZE`, получает отдельный SMB FileId, но остаётся
полностью логическим: он не сбрасывает `Copying`, не заменяет активный
`Last: WRITE` и не переоткрывает FILEX. Закрытие такого неактивного handle не
ждёт физическую очередь WRITE другого handle.
UART принимает
...
Финальные время и атрибуты сохраняются вместе с SMB-handle и применяются после
данных на CLOSE/FLUSH. Изменение EOF, в отличие от них, завершается физически
до успешного ответа соответствующего SET_INFO. Статические
FILE_FS_ATTRIBUTE/DEVICE/CONTROL возвращаются
