...
Результат: `SMB Server/build/ZIFISMB.WMF` v0.5.6. Прошивка
`s3-native-0.6.60` с выключенным диагностическим кольцевым журналом
...
записи. Каждый отложенный READ сначала получает отдельный `STATUS_PENDING` с
кредитом и `AsyncId`, затем финальный ответ с тем же `AsyncId`; это сохраняет
запрос живым, пока ESP собирает 64 КиБ через несколько медленных FILEX-окон.
