...
- два межъядерных кольца по 64 КиБ в PSRAM для `Network -> VFS` и
  `VFS -> Network`;
- общий пул из шестнадцати 64-КиБ рабочих PSRAM-слотов для SMB READ/WRITE и
...
SMB объявляет Windows сетевые запросы по 64 КиБ. Единственный READ, который
физически обслуживает последовательный FILEX-канал, получает interim
`STATUS_PENDING`, credit и `AsyncId`. Следующие запросы сохраняются маленькими
дескрипторами без interim-ответа и продолжают удерживать свои SMB credits.
После финального ответа старейший запрос получает буфер и PENDING, а Windows
может прислать ровно один следующий READ. Поэтому глубина серверной очереди
остаётся постоянной при любом числе блоков файла; далёкий хвост не превращается
в многоминутный async-запрос. Core 1 проводит каждый READ через физические окна
Wild Commander по 16 КиБ; каталожный кэш остаётся в отдельной области PSRAM.
...
Результат: `SMB Server/build/ZIFISMB.WMF` v0.5.7. Прошивка
`s3-native-0.6.81` с выключенным диагностическим кольцевым журналом
...
записи. Долгий READ получает `STATUS_PENDING` с кредитом и `AsyncId` только при
входе в единственный физический FILEX-сервис, затем финальный ответ с тем же
`AsyncId`. Следующие READ хранят только маленькие дескрипторы и до продвижения
не возвращают interim-credit. Каждый финал сдвигает окно ровно на один запрос,
поэтому Windows не может заранее превратить все блоки большого файла в
многоминутную async-очередь. Размер файла не связан ни с числом PSRAM-слотов,
ни с числом одновременно ожидающих READ.
...
Финальный `CLOSE` также занимает место в этой FIFO. Даже если обычный WRITE уже
получил ранний `SUCCESS` после копирования payload в PSRAM, `CLOSE` ждёт его
физической записи на SD. Версия `s3-native-0.6.80` устранила
...
Фиксированное окно из четырёх кредитов покрывает обязательный минимум
Windows и составные запросы Проводника. Для WRITE весь 64-КиБ payload сначала
сохраняется в отдельном PSRAM-слоте. Обычный WRITE подтверждается сразу полной
длиной и дописывается в фоне до CLOSE/FLUSH; WRITE_THROUGH подтверждается
только после SD. Сервер подтверждает заранее не более двух ещё не записанных
на SD блоков, а следующие ответы и кредиты удерживает как обратное давление.
Поэтому клиент не может переполнить фиксированный кэш, а физический хвост перед
`FLUSH`/`CLOSE` не превышает 128 КиБ. Это даёт раннее завершение порций CopyFile
без короткого SMB-ответа и без тайм-аута Windows. Заданный Проводником до первой записи
