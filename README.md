...
### Способ 1. Через esptool.py (рекомендуется)

Зажмите кнопку **BOOT** на модуле ESP32-S3-Zero, подключите кабель USB-C к компьютеру, отпустите кнопку BOOT и выполните команду:

```powershell
esptool.py --chip esp32s3 write_flash 0x0 firmware\firmware.factory.bin
```

*(Если порт COM не определился автоматически, укажите его явно, например `-p COM3`).*

### Способ 2. Через PlatformIO
