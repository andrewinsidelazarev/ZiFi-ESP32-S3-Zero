...
SMB объявляет Windows сетевые запросы по 64 КиБ. Медленные READ асинхронны
внутри ESP, но синхронны на проводе: interim `STATUS_PENDING` не выдаётся, и
каждый из четырёх принятых запросов удерживает свой SMB-credit до финального
ответа. Поэтому после отмены копирования Windows не может бесконечно пополнять
очередь за счёт interim-credit. Core 1 проводит каждый READ через физические
окна Wild Commander по 16 КиБ; каталожный кэш остаётся в отдельной области
PSRAM.
...
`s3-native-0.6.82` с выключенным диагностическим кольцевым журналом
...
записи. Долгий READ остаётся неблокирующим внутри ESP, но синхронным на проводе:
он не получает `STATUS_PENDING` и удерживает свой credit до финального ответа.
Поэтому нажатие «Отмена» не создаёт бесконечную очередь новых READ за счёт
interim-credit; уже принятая работа ограничена четырьмя кредитами, а точный
`CANCEL` обрабатывается по обычному `MessageId`. Размер файла не связан ни с
числом PSRAM-слотов, ни с числом одновременно ожидающих READ.
