...
## Совместимые программы Z80

Для проверки `zifi.spg` используется Native-версия из проекта
`ZiFi ESP-01S`. Она работает через двоичные команды
`NET_HTTP_GET/NET_RECV/NET_CLOSE`.

FTP запускает плагин v0.12 из `FTP Server/build/ZIFIFTP.WMF` в этом проекте.
Плагин передаёт прошивке `FTP_START`, после чего открывается управляющий порт.
По умолчанию логин и пароль — `zx` / `zx`; они находятся в теле команды
плагина, а не зашиты как неизменяемая настройка прошивки. Несколько control-
соединений нужны обычным FTP-клиентам для навигации и фоновой передачи; все
файловые операции при этом последовательно проходят через один VFS-тракт.
