...
`s3-native-0.6.67` с выключенным диагностическим кольцевым журналом
...
Windows и составные запросы Проводника. Для WRITE весь 64-КиБ payload сначала сохраняется в отдельном
...
ответы и кредиты удерживает как обратное давление. Поэтому клиент не может
переполнить фиксированный кэш, а физический хвост перед `FLUSH`/`CLOSE` не
превышает 128 КиБ. Это даёт раннее завершение порций CopyFile без короткого
SMB-ответа и без тайм-аута Windows. Заданный Проводником до первой записи
