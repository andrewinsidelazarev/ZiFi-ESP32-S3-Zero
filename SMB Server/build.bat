...
rem Этот путь вычисляется из каталога SMB Server\src.
...
"%SJASM%" --inc="%SHARED_Z80%" --sym=..\build\ZIFISMB.sym --lst=..\build\ZIFISMB.lst main.asm
if errorlevel 1 exit /b 1

echo Built: %~dp0build\ZIFISMB.WMF
