...
; CRC16-CCITT (полином #1021, начальное значение #FFFF, без отражения) —
; тот же, что считает crc16CcittFalse на стороне ESP.
;
; Вход: HL — данные, BC — длина. Выход: DE — CRC. Портит A, BC, HL.
;
; Считается по байту через таблицу, а не по биту. Побитовый вариант стоил
; 508 тактов на байт: на окне FILEX в 16352 байта это 8,3 млн тактов, то есть
; 1,19 с при 7 МГц — почти половина от 2,7 с, которые уходили на одно окно,
; и именно из-за него скорость упиралась в 6 КБ/с в обе стороны. Табличный
; проход стоит 86 тактов на байт, то есть около 0,2 с на окно.
;
; Замена алгоритма честная: значение то же самое. Индекс равен старшему байту
; CRC, сложенному XOR с байтом данных; новый старший байт — младший байт CRC
; XOR со старшей половиной табличной записи, новый младший — её младшая
; половина. Таблицы строит Vfs_Crc16Init при старте плагина.
Vfs_Crc16:
        ld de,#FFFF
        ld a,b
        or c
...
; значения i<<8 по тому же полиному. Стоит 256 проходов по восемь бит, около
; 19 мс при 7 МГц, и платится один раз за запуск плагина.
Vfs_Crc16Init:
