...
; Общий модуль обеих заставок погоды (TS-Conf и VDAC2).
;
...
; Запись таблицы WMO для кода A. Выход: HL — запись
; (код, иконка днём, иконка ночью, адрес описания). Последняя запись с кодом
; #FF подходит любому коду и служит значением по умолчанию. Саму таблицу
; WmoTable генерирует tools/gen_assets.py каждой заставки в strings.inc.
Wmo_Lookup:
        ld hl,WmoTable
.next:
        cp (hl)
        ret z                           ; нашли свой код
        ld e,(hl)
        inc e                           ; код #FF: конец таблицы (#FF+1 = 0, Z=1)
        ret z
        ld de,5                         ; 5 байтов на запись
        add hl,de
        jr .next
