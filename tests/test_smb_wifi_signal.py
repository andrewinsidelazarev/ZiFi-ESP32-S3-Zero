...
class WifiSignalTest(unittest.TestCase):
    ...
    def test_ftp_plugin_replaces_listening_with_signal_bar(self) -> None:
        plugin = (ROOT / "FTP Server" / "src" / "ftp_server.asm").read_text(
            encoding="utf-8"
        )
        ui = (ROOT / "FTP Server" / "src" / "ui.asm").read_text(
            encoding="utf-8"
        )
        handler = plugin[
            plugin.index("FtpEvent_WifiSignal:") : plugin.index("; --- данные")
        ]
